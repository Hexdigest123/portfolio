...
import { BASE_URL } from "@/components/statics";
...
export default function BlogList() {
          ...
          blogData.map((item, index) => {
            if (!item.image) {
              return (
                ...
                >
                  <Link href={`${BASE_URL}/blog/${item.slug}`}>
                    <img
                      src={item.image}
  ...
  if (loading) {
    return (
      <section className="mx-auto w-24">
        <svg
          aria-hidden="true"
        ...
        {blogArticles.length === 0 ? (
          <div>
            <p className="text-xl text-center mb-6">
              Currently, there are no public articles available!
            </p>
            <button
              type="button"
              onClick={() => {
